...
func RenderMarkdownReport(results []ScenarioResult, runAt time.Time) string {
	...
	sb.WriteString("# P0-Plan-Probe｜预案检查规则运行报告\n\n")
	sb.WriteString("## 1. 运行信息\n\n")
	sb.WriteString(fmt.Sprintf("- 运行时间：%s\n", runAt.Format("2006-01-02 15:04:05")))
	sb.WriteString("- 规则：FORMAL-RISK-001｜仓位动作前必须完成预案检查\n")
	...
	for _, result := range results {
		...
		if scenario.Note != "" {
			...
			sb.WriteString("- 解释说明：按 P0 plan_gate 优先级命中对应预案完整性检查。\n")
		}
		sb.WriteString("\n")
	}
	...
	if failCount == 0 {
		sb.WriteString("- 是否证明 plan_gate 最小闭环可运行：是\n")
	} else {
		sb.WriteString("- 是否证明 plan_gate 最小闭环可运行：否\n")
	}
	sb.WriteString("- 后续限制：本报告仅验证预案检查闸门 plan_gate 的 P0 Mock 场景。不接真实行情。不接真实账户。不构成投资建议。不生成买卖信号。不判断估值。不判断趋势破位。\n")
	return sb.String()
}

func outputEqual(a, b PlanGateOutput) bool {
...
func inputSummary(input PlanGateInput) string {
	...
	if isTrue(input.PlanChangedAfterAction) {
		parts = append(parts, "plan_changed_after_action=true")
	}
	if isTrue(input.EmotionalOverrideDetected) {
		parts = append(parts, "emotional_override_detected=true")
	}
