...
投资知识库自动整理 CLI。**当前阶段：模块一 V1.5.3｜稳定版（冻结）**。

> ⚠️ V1.5.3 已冻结目录结构、文件命名、链接规则、source_meta、文件模板、validate 规则。
> 详见 [`docs/FREEZE_V1.5.3.md`](docs/FREEZE_V1.5.3.md)。
...
RAW、QA、候选规则均写入独立 Markdown 文件，并维护各自索引。
...
- `00-待处理材料/问答`：用户手动放入的原始输入文档，V1.5 暂不批量扫描、不自动移动。
- `01-源文档/问答`：程序生成 RAW 独立文件和原始材料索引，不再作为手动 inbox。
